...
def evaluate(exp): #替换，识别，运算
    ...
    for ops in [
        ...
        for op in ops: #op:['AND',2(双目运算符)]
            for i in range(exp.count(op[0])):
                if op[1]==1 and op[0] in exp: #处理单目运算符,要求存在该运算符+
                    py_op=op[0].lower()
                    index=seps.index(op[0])
                    replace_str=str(eval('%s evaluate(seps[index+1])'%py_op)) #eval('not 1') -> 'False'
                    seps=seps[:index]+[replace_str]+seps[index+2:]
                    #为了允许多次处理，再次转化为str
                if op[1]==2 and op[0] in exp: #处理双目运算符,要求存在该运算符
                    py_op=op[0].lower()
                    index=seps.index(op[0])
                    replace_str=str(eval('evaluate(seps[index-1]) %s evaluate(seps[index+1])'%py_op)) #eval('1 AND True')
